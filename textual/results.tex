...
Em razão disso, é apresentado, nesta seção, um \alert{estudo de caso}, em que os resultados de uma simulação do problema de lançamento oblíquo são mostrados e comparados com a respectiva solução analítica. Também são expostas análises relativas à influência da alteração de parâmetros da simulação no erro.
...
\begin{figure}[h]
	\caption[Erro da simulação com relação à solução numérica para a posição e da partícula]{Erro da simulação com relação à solução numérica para (\subref{subfig:position_error}) a posição e (\subref{subfig:velocity_error}) da partícula}
	% \vspace{-0.5cm}
	\centering
	\captionsetup[subfloat]{labelfont=bf}
	\subfloat{
...
% \alert{Verificar se é + ou -} --> É -, pois a velocidade da partículas seria maior se não houvesse a gravidade
