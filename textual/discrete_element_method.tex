...
% 			\centering
% 			\includegraphics[width=0.5\textwidth]{images/discrete_element_method/geometries/sphere.pdf}
% 			\caption{Geometria esférica.}
% 			\label{subfig:element_geometry:sphere}
% 		\end{subfigure}
% 		\begin{subfigure}[t]{\smallresultsfigwidth}
% 			\centering
% 			\includegraphics[width=\textwidth]{images/discrete_element_method/geometries/plane.pdf}
% 			\caption{Geometria plana.}
% 			\label{subfig:element_geometry:plane}
% 		\end{subfigure}
% 		\begin{subfigure}[t]{\smallresultsfigwidth}
% 			\centering
% 			\includegraphics[width=0.5\textwidth]{images/discrete_element_method/geometries/superelipsoid.pdf}
% 			\caption{Geometria superelipsoidal.}
% 			\label{subfig:element_geometry:ellipsoid}
% 		\end{subfigure}
% 		\begin{subfigure}[t]{\smallresultsfigwidth}
% 			\centering
% 			\includegraphics[width=0.5\textwidth]{images/discrete_element_method/geometries/polyhedron .pdf}
% 			\caption{Geometria poliédrica.}
% 			\label{subfig:element_geometry:polyhedron}
% 		\end{subfigure}
% 		% \begin{subfigure}[t]{\smallresultsfigwidth}
% 		% 	\centering
% 		% 	\includegraphics[width=\textwidth]{images/introduction/composite_particle.png}
...
\begin{figure}[h]
	...
	\centering
		\input{tikzpictures/discrete_element_method/general_algorithm}
...
De acordo com \citeonline{bib:computational_granular_dynamics}, a inicialização é comumente feita por meio de um arquivo de entrada, muito embora programas de simulação geralmente apresentem uma interface gráfica com essa finalidade. 
...
% é estranho colocar a vizinhança antes. Três motivos por que isso é feito: 1) a referência faz; 2) o passo de tempo que usamos é pequeno; 3) permite uma fácil alteração do algoritmo de integração da equação
...
\begin{equation*}
	...
	\end{array},
	% \quad \particlei \in \sphericalParticleSet, \elementj \in \neighborhoodi.
	\quad
	\def\arraystretch{1}
	\begin{array}{c}
...
Diversos autores sugerem a limitação do passo de tempo da simulação para se garantirem a estabilidade numérica e a representatividade das soluções. Segundo \citeonline{bib:gomes2014}, uma possível estratégia é definir-se um valor de \textit{passo de tempo crítico} \(\criticalDt\) com a forma
...
Na ausência de melhores formulações, passos de tempo arbitrados acabam sendo utilizados nas simulações, e sua obtenção baseia-se fortemente na intuição do usuário, em cálculos prévios e na experimentação numérica.
...
\begin{figure}[h]
	...
	\subfloat{
		...
		\begin{subfigure}[t]{\smallresultsfigwidth}
			\centering
			\input{tikzpictures/discrete_element_method/one_way}
			\caption{\textit{One-way}}
			\label{subfig:method_coupling:flowchart:one-way}
		\end{subfigure}
		\begin{subfigure}[t]{\smallresultsfigwidth}
			\centering
			\input{tikzpictures/discrete_element_method/two_way}
