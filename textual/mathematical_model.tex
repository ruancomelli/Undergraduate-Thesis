...
\subsubsection*{Coeficientes de Restituição} 

A determinação dos coeficientes de restituição tem a vantagem de poder simplificar e reduzir o tempo de simulação em simulações orientadas a eventos, mas também pode ser usada para a validação de métodos numéricos baseados no cálculo das forças, que são o foco deste trabalho.

Para cada par de elementos, os coeficientes de restituição relacionam o estado posterior ao choque ao estado anterior. Escrevendo a velocidade do elemento \(j\) relativa ao \(i\) imediatamente antes da colisão como \(\beforeCollision{\relativeVelocityContactPointij}\), e \(\afterCollision{\relativeVelocityContactPointij}\) como a velocidade correspondente ao final da colisão, definem-se os coeficientes de restituição na direção normal \(\normalCoefficientOfRestitutionij\) e na direção tangencial \(\tangentialCoefficientOfRestitutionij\) através das equações
\begin{gather*}
	\afterCollision{\relativeNormalVelocityScalarij} = - \normalCoefficientOfRestitutionij\beforeCollision{\relativeNormalVelocityScalarij}\text{, e} \\
	\afterCollision{\relativeTangentialVelocityScalarij} = - \tangentialCoefficientOfRestitutionij\beforeCollision{\relativeTangentialVelocityScalarij}.
\end{gather*}
...
O modelo para esferas viscoelásticas tem a finalidade de expressar de forma mais fidedigna os termos elástico e dissipativo.
...
No início do choque, tanto a superposição quanto a sua derivada são positivas o que implica, de acordo com as expressões para a força, que ocorre repulsão entre as partículas. A penetração entre as partículas aumenta continuamente até que, em determinado instante, atinge um valor máximo. Devido à força normal repulsiva, inicia-se o fenômeno de descompressão, em que as partículas se afastam. Nessa etapa, a derivada da superposição é negativa. É possível, dependendo dos parâmetros da simulação, que essa diferença de sinais entre a superposição e a sua derivada resultem em uma força normal \textit{atrativa}. Essa atração ocorre quando \(\overlapDerivativeij\) atinge um valor crítico \(\principal{\overlapDerivativeij}\), em que a força calculada é nula.
...
\begin{figure}[h]
	...
	\legend{Fonte: \citeonline[p. 22]{bib:computational_granular_dynamics}.}
	% \vspace{-1cm}
\end{figure}

No entanto a atração entre partículas geralmente não condiz com resultados experimentais. Uma das técnicas para eliminar esse fenômeno é limitarem-se inferiormente as forças calculadas. Esse artifício justifica-se pois, embora tenha sido feita a hipótese de que as partículas não se deformem, partículas reais são deformáveis, e essa deformação pode ser suficiente para encerrar a colisão em um tempo anterior do que aquele considerado pelo modelo que partículas rígidas.

Isso é representado na figura \ref{fig:particle_attraction}.
...
\begin{equation*}
	...
	\left\lbrace
		...
		\begin{array}{ll}
			\exp\bqty{-\dfrac{\beta}{\omega}\pqty{\pi - \arctan\dfrac{2\beta\omega}{\omega^2-\beta^2}}}, & \text{se } \beta < \dfrac{\omega_0}{\sqrt{2}} \\
			\exp\bqty{-\dfrac{\beta}{\omega}\arctan\dfrac{2\beta\omega}{\omega^2-\beta^2}}, & \text{se } \beta \in \bqty{\dfrac{\omega_0}{\sqrt{2}}, \omega_0} \\
			\exp\bqty{-\dfrac{\beta}{\Omega} \ln\dfrac{\beta + \Omega}{\beta - \Omega}}, & \text{se } \beta > \omega_0
		\end{array}
	\right.
	,
\end{equation*}
em que
\begin{gather*}
	\omega_0 = \bigslant{\effectiveNormalElasticij}{\massij}, \\
...
Modelos para forças tangenciais, diferentemente dos modelos para forças normais, têm caráter intrinsecamente dinâmico, resistindo sempre ao movimento relativo entre as partículas. Modelos de força tangencial são responsáveis por fornecer um valor de força tangencial
\begin{equation*}
	\tangentialForceji = \tangentialForceScalarji\tangentialVersorji.
\end{equation*}

O fato de que a força tangencial resiste ao movimento é representado pela desigualdade
\begin{equation*}
	\tangentialForceScalarji \leq 0.
\end{equation*}

Ainda, plea lei de Coulomb, a força tangencial não pode superar, em módulo, a força de atrito dinâmico 
\begin{equation*}
...
Segundo \citeonline{bib:computational_granular_dynamics}, o modelo de Haff e Werner já foi aplicado com sucesso em diversas simulações. Esse modelo é o análogo tangencial do modelo de amortecedor linear para a força normal.

Para partículas esféricas, supondo que há contato, resulta das equações \eqref{eq:spherical_particle_overlap_derivative} e \eqref{eq:relative_normal_velocity} que
\begin{equation*}
...
em que
\begin{itemize}
	\item \(\tangentialDampingConstantij\) a constante de amortecimento efetiva do par \(i\) e \(j\). Essa constante pode ser obtida por uma associação em série de amortecedores de constantes \(\tangentialDampingConstanti\) e \(\tangentialDampingConstantj\), cada um associado a uma das partículas, como
	\begin{equation*}
...
Com isso, o modelo de Haff e Werner assemelha-se a um modelo de amortecedor linear, exceto pelo fato de haver a limitação da lei de Coulomb.
...
\begin{equation*}
	\tangentialForceScalarji = - \min\Bqty{\tangentialElasticConstantij\elongationij\pqty{t},\,\, \dynamicFrictionij\cdot\normalForceScalarji},
\end{equation*}
sendo que
\begin{itemize}
	\item \(\tangentialElasticConstantij\) é a constante elástica da mola equivalente no choque. Essa mola equivalente pode ser tratada como a associação em série de molas de constantes \(\tangentialElasticConstanti\) e \(\tangentialElasticConstantj\):
	\begin{equation*}
		\tangentialElasticConstantij = \tangentialElasticConstanti + \tangentialElasticConstantj.
	\end{equation*}
...
Com as equações diferenciais bem determinadas através do uso de modelos de interação, é possível buscar-se a sua solução. Nos métodos computacionais para a solução dessas equações, a extrapolação de funções possui um papel fundamental por permitir a estimativa de valores além do conjunto previamente conhecido.

Para simplificação da notação, dada uma função \(y: X\to Y\), define-se o vetor das \(k\) primeiras derivadas de \(y\) como a função vetorial
\begin{equation*}
...
\begin{equation}
	\drvec{\taylorOrder}{\predicted{y}} = \mtx{A} \cdot \drvec{\taylorOrder}{y}.
\end{equation}
...
O método de extrapolação por expansão de Taylor combina simplicidade, exatidão e estabilidade, e por isso é o método adotado nas etapas posteriores deste trabalho. Esse método é fundamentado pelo \nameref{theo:taylor}.
...
\begin{equation} \label{eq:position_prediction}
	\predicted{\position}\pqty{t+\Dt} = \position\pqty{t} + \Dt\cdot\deriv{1}{\position}\pqty{t} + \dfrac{\Dt^2}{2}\cdot \deriv{2}{\position}\pqty{t} + \dots + \dfrac{\Dt^\taylorOrder}{\taylorOrder!}\cdot \deriv{\taylorOrder}{\position}\pqty{t}.
\end{equation}
...
A matriz da equação \eqref{eq:position_prediction} é a matriz de extrapolação do método de expansão de Taylor, sendo a mesma para a extrapolação de ordem \(\taylorOrder\) para qualquer função.

Esse método ainda pode ser aplicado à função de orientação da partícula e a outros graus de liberdade que o problema porventura exija.
...
\[\drvec{\taylorOrder}{\predicted{y}}\pqty{t+\Dt} = \mtx{A}\cdot\drvec{\taylorOrder}{y}\pqty{t}\]
...
Para o caso de uma partícula, tem-se \(p=2\) e \(q=1\) para as equações de movimento, o que resulta em um erro da ordem de \(\Dt^{\taylorOrder}\).

Com essas informações, todas as ferramentas matemáticas necessárias para a solução do problema já estão expostas. A próxima etapa, então, é desenvolver um método numérico que permita a implementação desse algoritmo de solução.