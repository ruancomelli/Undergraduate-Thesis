...
Na dinâmica de partículas, os elementos estudados são considerados corpos rígidos, aos quais aplicam-se a Segunda Lei de Newton \eqref{eq:second_law_of_motion} e a equação de Euler \eqref{eq:euler_equation} \citeay{bib:computational_granular_dynamics}.

Seja \particle{} uma partícula de massa \(\mass\) cuja posição é descrita, em função do tempo, pela função \(\position\).  \alert{Como abordar a questão dos ângulos de Euler?}

A Segunda Lei de Newton afirma que
\begin{equation} \label{eq:second_law_of_motion}
	\resultingForce = \mass\cdot\deriv{2}{\position},
\end{equation}
sendo \(\resultingForce\) o vetor força resultante sobre a partícula. Uma forma mais geral da Segunda Lei de Newton é 
\[
	\resultingForce = \deriv{1}{\linearMomentum},
\]
em que \(\linearMomentum = \mass\cdot\deriv{1}{\position}\) é o \textit{momento linear} da partícula. A equação na forma \eqref{eq:second_law_of_motion} é mais adotada em virtude da sua simplicidade.
...
\begin{equation} \label{eq:taylor_trunc}
    y(t + \Dt) \cong y(t) + \deriv{1}{y}(t)\cdot\Dt + \dots + \dfrac{\deriv{\taylorOrder}{y}(t)}{\taylorOrder!}\cdot\Dt^\taylorOrder.
\end{equation}
