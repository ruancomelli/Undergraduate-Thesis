...
Este capítulo aborda os aspectos matemáticos pertinentes ao método. São abordados os principais conceitos referentes aos sistemas de partículas, e apresentadas as equações de movimento que governam o problema. E, por fim, é apresentado o algoritmo de Gear, responsável por resolver as equações.
...
Em um sistema inercial fixo, considera-se uma partícula de massa \(\mass\) com um centro de massa cuja posição é descrita pela função vetorial \(\position\). Além disso, considera-se que a partícula possua um tensor momento de inércia \(\tensorOfInertia\) e uma velocidade angular \(\angularVelocity\) definidos com relação à origem do sistema.

\alert{Fazer apêndice explicando rotações em três dimensões}
...
\begin{equation*}
	\angularMomentum = \tensorOfInertia\cdot\angularVelocity.
\end{equation*}
...
\begin{equation} \label{eq:motion_second}
	\resultingTorque = \deriv{1}{\angularMomentum} = \tensorOfInertia\deriv{1}{\angularVelocity} + \angularVelocity\cross\pqty{\tensorOfInertia\angularVelocity}.
\end{equation}

Novamente, a hipótese de que a massa da partícula é constante é geralmente considerada. 

Entretanto, mesmo diante de situações em que \(m\) é constante, a resolução da equação \eqref{eq:motion_second} possui algumas complicações em simulações tridimensionais. Isso se deve ao fato de que são necessárias parametrizações para a orientação, e geralmente não ocorre que a velocidade angular seja a derivada desses parâmetros. Mais detalhes sobre rotações em três dimensões são apresentados no \autoref{app:three_dimensional_rotation}.

Por simplicidade, considera-se aqui o caso de simulações bidimensionais. Convencionando que os movimentos se dão apenas no plano \(\xAxis\yAxis\), a orientação de uma partícula pode ser representada por uma rotação de \(\orientationScalar\) em torno do eixo \(\zAxis\). Denotando por \(\xUnit\), \(\yUnit\) e \(\zUnit\) a base canônica para o espaço \(\real^3\), segue, nesse caso, que
\[
	\angularVelocity = \angularVelocityScalar\zUnit = \deriv{1}{\orientationScalar}\zUnit
\]
\alert{Será?? Só simulações bidimensionais?}
